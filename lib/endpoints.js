...
const debug = require('debug')('burnbox:main');
...
app.get(`${ config.baseUrl }files/:fid`, async (req, res, next) => {
  ...
  if (req.params.fid.match(/^[a-z0-9+]+\.zip$/)) {
    ...
    const expectedLegacy = `${ sid }++${ legacyMd5 }.zip`;
    const expectedNew = `${ sid }++${ newSha256 }.zip`;
